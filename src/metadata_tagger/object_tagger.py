...
def tag_file(key: str, bucket: str, extraction_function: Callable[[str], Dict[str, str]]) -> Dict[str, str]:
    """
    ...
    while not result and retries < 2:
        ...
        file = _download_file(key, bucket)  # todo open with
        custom_tags = extraction_function(file.name)
...
def _download_file(key: str, bucket: str) -> IO[bytes]:
